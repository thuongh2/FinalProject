...
def create_model_relationship():
    if request.method == 'POST':
        ...
        model_valid = model_relationship_collection.find_one(
        ...
        if (model_valid):
            ...
            model_relationship_collection.insert_one(model_relationship)
            flash('Thêm mô hình thành công')

    model_relationships = model_relationship_collection.find()
    try:
        model_info = cache['model_info']
    except:
        model_info = model.find()    
...
def delete_model_relationship(model_id):
    model_relationship_collection.delete_one({'_id': model_id})
...
def get_traning_from_model(model_name):
    ...
    traning_model = list(train_model.find(query))
